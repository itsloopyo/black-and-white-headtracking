...
<#
.SYNOPSIS
    Configure, build and run the host-side unit tests.
.DESCRIPTION
    The test targets are behind HEADTRACKING_BUILD_TESTS, which is OFF in the build
    tree `pixi run build` uses, so they get their own Win32 tree in build-tests/ and
    the shipped DLL in build/ is never built out of a tree configured for tests.
...
& cmake -S $projectRoot -B $buildDir -A Win32 -DHEADTRACKING_BUILD_TESTS=ON
