...
// fresh render, the defaults v0.2.1 ran on map to the defaults, a first start creates the
// committed file, the toggles save only their own lines and leave Defaults.ini and
// HeadTracking.ini alone, End's row cannot be saved, and a folder the ANSI code page cannot name
// imports as v0.2.1 read it.
//
...
std::string Committed() {
    return ReadBytes(fs::path(BW_COMMITTED_CONFIG));
}
...
struct Scratch {
    ...
    explicit Scratch(const std::wstring& gameFolder) {
        ...
        root = fs::path(temp) / (L"bw-config-tests-" + std::to_wstring(GetCurrentProcessId()));
...
void TestFirstStartCreatesTheCommittedFile() {
    ...
    Check(ReadBytes(s.ConfigPath()) == Committed(), "a first start creates config/HeadTracking.ini's bytes as CameraUnlock.ini");
...
void TestLegacyDefaultsMapToTheDefaults() {
    wchar_t temp[MAX_PATH];
    GetTempPathW(MAX_PATH, temp);
    const fs::path missing = fs::path(temp) / L"bw-no-such-folder" / kLegacyConfigFileName;
    ...
    Check(AllValues(mapped) == AllValues(table.defaults()), "v0.2.1's defaults map to the defaults");
...
// v0.2.1 built its path from GetModuleFileNameA, so in a folder the ANSI code page cannot name
// it looked for a file that does not exist and ran on its defaults. The import reads the same
// ANSI path, so such a player migrates to the defaults, and HeadTracking.ini stays as it was.
void TestAFolderTheCodepageCannotNameImportsAsV021ReadIt() {
    ...
    Check(loaded.status == cfg::ConfigLoadStatus::Migrated, "a file v0.2.1 could not find migrates to its defaults");
    Check(loaded.config.udp_port == 4242, "the port is v0.2.1's default, as that build ran");
...
int main(int argc, char** argv) {
    try {
        ...
        TestTogglesSave();
        TestAFolderTheCodepageCannotNameImportsAsV021ReadIt();
