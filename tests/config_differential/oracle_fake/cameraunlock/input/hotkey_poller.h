...
// Stands in for core's HotkeyPoller in the hotkey oracle library only, so v0.2.1's
// HotkeyHandler::Start (oracle/src/hotkey_handler.cpp, verbatim) runs with no polling thread
// and the test sees what it registered. Compiled with that library's renaming, so everything
// here lives in bw_oracle_hk_core and cannot collide with the real poller the current core
// links.
//
// The GetAsyncKeyState below is declared in the namespace chord_hotkeys.h's guards are defined
// in, so their unqualified calls find it before ::GetAsyncKeyState. That is how the test holds
// modifiers without a keyboard.
